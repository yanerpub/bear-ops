    ...
    <div>
        <form>
            ...
            <div class="form-group row" v-for="field in fields">
                ...
                <div class="col-xs-10">
                    <input v-if="field.type == 'text'" class="form-control" type="text" v-model="product.fieldMap[field.key]" :required="field.required" disabled>
                    <input v-if="field.type == 'password'" class="form-control" type="password" v-model="product.fieldMap[field.key]" :required="field.required" disabled>
                    <textarea rows="3" v-if="field.type == 'textarea'" class="form-control" v-model="product.fieldMap[field.key]" :required="field.required" disabled></textarea>
                    <checkbox v-if="field.type == 'checkbox'" :field-key="field.key" :default-value="product.fieldMap[field.key]" :optional-values="field.optionalValues" :disabled="disabled"></checkbox>
                    <div v-if="field.type == 'radio'">
                        <label class="form-check-inline" v-for="op in field.optionalValues">
                            <input class="form-check-input" type="radio" :name="field.key" :value="op.value" v-model="product.fieldMap[field.key]" disabled> {{op.name}}
                        </label>
                    </div>
            ...
            <router-link class="btn btn-secondary" :to="{ name: 'hotelSupplier', params: { sid: $route.params.sid }}">返回</router-link>
        </form>
    </div>
</template>
